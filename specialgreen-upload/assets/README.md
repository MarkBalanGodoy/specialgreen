...
**Photographs live in `assets/images/`, not here.**

The homepage is wired to every filename in `docs/SHOT-LIST.md`. Drop a real photograph into
`assets/images/` under the matching name and it appears on the site with no code change. The
dark placeholders sitting there now are neutral tone with no depicted content, so the page is
never broken and never implies a project the company did not do.

Run `tools/prepare-images.sh` on the originals first. It resizes, compresses, and strips the
GPS coordinates that photos taken at customers' homes carry.
